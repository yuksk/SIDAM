...
Static Function scalingLineProfile(STRUCT paramStruct &s, Wave linew, Wave sdevw)
	...
	String noteStr
	Sprintf noteStr, "src@%s;start@p=%s,q=%s;end@p=%s,q=%s;width=%s", GetWavesDataFolder(s.w, 2)\
		, num2str(s.p1), num2str(s.q1), num2str(s.p2), num2str(s.q2), num2str(s.width)
	Note linew, noteStr
	Note sdevw, noteStr
End
...
Static Function pnl(String grfName, String imgName)
	...
	SIDAMApplyHelp(pnlName, "[SIDAM_LineProfile]")
