...
Static Function setxyStr(String &xys, STRUCT SIDAMMousePos &ms, String grfName)
	...
	String formatStr = constructFormatStr(grfName, SIDAM_WINDOW_FORMAT_SHOWUNIT, ms.w)
	Variable mag
	...
	switch (mode)
		case 0:		//	x, y	(also for traces)
			...
			if (WaveDims(ms.w) == 1)
				Wave/Z xw = XWaveRefFromTrace(grfName,NameOfWave(ms.w))
				if (WaveExists(xw))
					xUnit = WaveUnits(xw,-1)
				endif
			endif
			if (stringmatch(xunit,"dat"))
...
Static Function/S constructFormatStr(String grfName, int showunit, Wave/Z w)
	if (!WaveExists(w))
		return ""
	endif

	int mode = str2num(GetUserData(grfName,"","mode"))
	if (mode < 0 || mode > 2)		//	e.g., NaN for Fourier filter
		mode = 0
	endif
	...
	if (mode == 0)	//	x, y
		if (WaveDims(w) == 1)
			Wave/Z xw = XWaveRefFromTrace(grfName,NameOfWave(w))
			if (WaveExists(xw))
				xUnit = WaveUnits(xw,-1)
			endif
		endif
