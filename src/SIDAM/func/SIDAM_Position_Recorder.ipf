...
Static Function pnlHook(STRUCT WMWinHookStruct &s)
	switch (s.eventCode)
		...
		case 14:	//	subwindowkill
			...
			//	/Z is necessary because the wave is not found if thw parent
			//	window is closed.
			Wave/Z w = $StringByKey("WAVE", TableInfo(s.winName,0))
			if (!numpnts(w))
	...
	strswitch (s.ctrlName)
		case "startB":
			...
			if (V_Value)
				...
				SetDataFolder GetWavesDataFolderDFR(SIDAMImageNameToWaveRef(grfName))
