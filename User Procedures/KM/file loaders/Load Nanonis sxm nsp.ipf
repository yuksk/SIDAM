...
Static Function/S menu()
	...
	Wave/Z configw = $(":"+KM_DF_SETTINGS+":'Multipass-Config'")
...
Function KMNanonisConcatMultipass(DFREF dfr, [int history])
	...
	Wave/SDFR=dfr:$KM_DF_SETTINGS configw = 'Multipass-Config'
...
Static Function/WAVE init(DFREF dfr)
	...
	if (DataFolderRefStatus(dfr) != 1)
		...
		return $""
	elseif (DataFolderRefStatus(dfr:$KM_DF_SETTINGS) != 1)
		print errStr+"the settings datafolder is not found."
		return $""
	endif
	
	Wave/Z/SDFR=dfr:$KM_DF_SETTINGS configw = 'Multipass-Config'
