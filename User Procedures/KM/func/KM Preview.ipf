...
Static Function/WAVE getWaveRefList(dfr, str, option)
	...
	for (i = 0; i < n; i += 1)
		...
		if (WhichListItem(dfName, "_KM;pos;Packages;"+KM_DF_SETTINGS) != -1)	//	探索しないデータフォルダの名前
			Continue
		endif
...
Static Function updateVariablesList(STRUCT WMListboxAction &s)
	...
	DFREF dfr = GetWavesDataFolderDFR(ref[s.row]):$KM_DF_SETTINGS
...
Function/S KMGetSettings(Wave/Z w, int kind)
	...
	DFREF dfr = GetWavesDataFolderDFR(w):$KM_DF_SETTINGS
