...
Static StrConstant CTABGROUPS = "NistView;KM;Wolfram;Matplotlib1;Matplotlib2;IDL;CET;"
...
Static Function pnl(String grfName)
	...
	int i, n
	int needUpdate = DataFolderExists(KM_DF_CTAB) ? NumVarOrDefault(KM_DF_CTAB+"needUpdate",1) : 1
	
	//	カラーテーブルが読み込まれていなければ読み込む
	if (needUpdate)
		loadColorTableWaves()
		Variable/G $(KM_DF_CTAB+"needUpdate") = 0
	...
	//	タブ
	String tabNameList = "Igor;" + CTABGROUPS + SelectString(DataFolderExists(KM_DF_CTAB+"Ext"),"","Ext")
...
Static Function findTabForPresentCtab(String grfName, String imgName)
	...
	if (CmpStr(ctabName[0],":"))
		...
		if (index != -1)
			...
			//	ctabName の中に KM_DF_CTAB(からrootを除いたもの)が含まれていれば、Extの場合に相当
			if (strsearch(RemoveListItem(0,KM_DF_CTAB,";"),ctabName,0) != -1)
...
Static Function pnlTabComponents(String pnlName, int tab, int hide)
	...
	if (tab == 0)	//	Igor標準
		...
		list = fetchWavepathList($(KM_DF_CTAB+StringFromList(tab-1,CTABGROUPS+"Ext")))
	endif
	int i, n
...
//	ibwFilePath = ***:User Procedures:KM:ctab:NistView:Autumn.ibw
//	の場合、ウエーブは
//	root:'_KM':ctable:NistView:Autumn
//	となる。
//-----------------------------------------------------------------------
Static Function/S loadColorTableWave(String ibwFilePath)
	
	//	ibwFilePath = ***:User Procedures:KM:ctab:NistView:Autumn.ibw
	//	colorFDpath = ***:User Procedures:KM:ctab
	//	extFDpath   = ***:User Procedures:KM:extension
	//	wPath       = :NistView:Autumn.ibw
	String colorFDpath = KMGetPath() + KM_FOLDER_COLOR
	String extFDpath = KMGetPath() + KM_FOLDER_EXT
	...
	//	既に読み込まれたものがあれば、終了
	Wave/Z ctabw = $(RemoveEnding(KM_DF_CTAB,":")+wPath)
	if (WaveExists(ctabw))
		return ""
	endif
...
//	使われていないウエーブをすべて削除する
//-----------------------------------------------------------------------
//	KM_DF_CTAB以下の使われていないウエーブを削除する
Static Function killUnusedWaves()
	
	//	KM_DF_CTAB が存在していなければ何もする必要がなく、終了
	if (!DataFolderExists(KM_DF_CTAB))
		return 0
	endif
	
	//	KM_DF_CTAB　以下の使われていないウエーブ・データフォルダを削除する
	killUnusedWavesHelper($KM_DF_CTAB)
	
	//	KM_DF_CTAB 以下に使われているデータフォルダが残っていなければ KM_DF_CTAB を削除して、終了
	if (!CountObjectsDFR($KM_DF_CTAB,4))
		KillDataFolder $KM_DF_CTAB
		//	KM_DF_CTAB を削除した結果 KM_DF 以下にデータフォルダが含まれないようになったら KM_DF を削除する
		if (!CountObjectsDFR($KM_DF,4))
			KillDataFolder $KM_DF
		endif
		return 0
	endif
	
	//	KM_DF_CTAB 以下にデータフォルダが残っている、つまり、使われているウエーブが残っている場合には、
	//	次回にカラーテーブルを選ぶパネルを開くときにウエーブを読み込む必要があるフラグを残しておく
	NVAR/SDFR=$KM_DF_CTAB/Z needUpdate
	if (NVAR_Exists(needUpdate))
		needUpdate = 1
	else
		Variable/G $(KM_DF_CTAB+"needUpdate") = 1
	endif
End
...
Static Function cindexWave2ctabWave()
	...
	//	旧バージョンではこの3つ
	String list0 = fetchWavepathList($(KM_DF_CTAB+"KM"))
	String list1 = fetchWavepathList($(KM_DF_CTAB+"NistView"))
	String list2 = fetchWavepathList($(KM_DF_CTAB+"Wolfram"))
